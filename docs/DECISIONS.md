...
- 2026-08-18 · 故事线步骤支持 `expand` 与 `clip` · 有了结构层级和剖切封盖之后，
  故事线才配叫"分镜"：心跳之旅从 7 步扩到 10 步，走到心脏时自动展开内部、点名二尖瓣、
...
- 2026-08-19 · **内容单元命名待定** · 「故事线」要改，用户否掉了旅程/导览/探秘/脉络
  四个候选，理由是"科普的意境不强"。改名会同时动 i18n、文件名、URL 与 UGC 界面，
  定下来之前代码里的 `Tour` 类型与 `content/tours/` 路径都不动。
