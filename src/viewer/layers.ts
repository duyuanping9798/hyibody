...
const CURVES: Record<SystemId, ReadonlyArray<readonly [number, number]>> = {
  ...
  // 内层的"底噪"从 layer=0 起步改成从 0.04 起步：皮肤换成不透明的物理材质之后，
  // layer=0 本来就看不见里面，底噪唯一的效果是让紧贴皮下的结构（大隐静脉那一类）
  // 从减面外壳外面冒出来，在腿上留一道红印。留一段纯皮肤，第一屏干净得多。
  muscles: [
    [0.0, 0],
    [0.04, 0.3],
    [0.2, 1],
  ...
  skeleton: [
    ...
    [0.04, 0.22],
    [0.45, 1],
    // 轮到器官时骨骼要快点让位：0.5 不透明度的肋骨压在器官前面，
    // 器官的配色会被洗成一片米白（人类反馈"看不出是什么器官"）
    [0.58, 0.35],
  ],
  organs: [
    [0.0, 0],
